...
def find_step():
    ...
    '''
    large_trunc_value = max(lambdas)
    for i in xrange(lambdas.size):
        if lambdas[i] == large_trunc_value:
            xp_num = i-1
    xp_file_1 = np.loadtxt('xp_%s.dat'%(xp_num))
    xp_file_2 = np.loadtxt('xp_%s.dat'%(xp_num+1))
    xp_file_3 = np.loadtxt('xp_%s.dat'%(xp_num-1))
    return xp_file_1.tolist(), xp_file_2.tolist(), xp_file_3.tolist()

def save_step(xp1, xp2, xp3):
    os.chdir('data_files')
    print 'SAVING DATA TO FILES'
    stepfile1 = open('param_steps_'+str(args.brs)+'[xp1].dat','a')
    stepfile2 = open('param_steps_'+str(args.brs)+'[xp2].dat','a')
    stepfile3 = open('param_steps_'+str(args.brs)+'[xp3].dat','a')
    for i in xrange(len(xp1)):
        stepfile1.write('%s '%(xp1[i]))
    for i in xrange(len(xp2)):
        stepfile2.write('%s '%(xp2[i]))
    for i in xrange(len(xp3)):
        stepfile3.write('%s '%(xp3[i]))
    os.chdir(cwd)
#############################FUNCTIONS##############################
